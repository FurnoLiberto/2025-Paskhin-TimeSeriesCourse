...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict) -> None:
    ...
    fig.add_trace(go.Scatter(x=np.arange(query_len), y=query, line=dict(color=query_color)),
                row=1, col=1)
    
    fig.add_trace(go.Scatter(x=np.arange(ts_len), y=ts, line=dict(color=ts_color)),
                row=1, col=2)

    # Мы итерируемся напрямую по списку индексов, который лежит в словаре по ключу 'indices'.
    # Так как bestmatch_results['indices'] уже содержит числа, преобразование int() не требуется.
    for match_index in bestmatch_results['indices']:
        
        # Создаем диапазон x для подпоследовательности
        match_x_range = np.arange(match_index, match_index + query_len)
        # Извлекаем саму подпоследовательность из временного ряда
        match_y_subsequence = ts[match_index : match_index + query_len]
        
        # Добавляем найденную подпоследовательность на второй подграфик
        fig.add_trace(go.Scatter(x=match_x_range, y=match_y_subsequence, line=dict(color=query_color)),
                      row=1, col=2)

    # Настройки отображения графика (остаются без изменений)
    fig.update_annotations(font=dict(size=24, color='black'))
